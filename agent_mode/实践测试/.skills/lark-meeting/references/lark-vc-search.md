...
# 通过 9 位会议号查询会议 ID
lark-cli vc +search --query "123456789" --format json
...
使用本命令需具备 `vc:meeting.search:read` 权限，由 agent 平台注入用户凭证。
...
| 权限不足 | 未授权 `vc:meeting.search:read` | 让 agent 平台为当前用户补开缺失的 scope 后重试 |
