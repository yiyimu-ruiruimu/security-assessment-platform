...
当已有 select 字段的选项名称、数量或业务含义要改变，且表中已有记录时，先完成数据迁移，再收敛字段定义：

1. 用 `+field-get` 确认旧 `options`，为每个旧选项写出到新选项的映射；没有明确映射的旧值不要静默丢弃。
2. 用 `+record-list` 读取该字段的旧值分布，保留 `record_id` 和该 select 字段；确认记录集完整或 `has_more=false` 后再迁移，若要同步计算字段，还同时读取公式依赖字段。
3. 先用 `+field-update` 添加新选项并暂时保留仍被记录引用的旧选项，避免平台在更新字段定义时清空不匹配的旧值。
4. 用 `+record-batch-update` 按映射把旧值写成新值；单选字段仍以数组形式提交，例如 `{"状态":["Done"]}`。
5. 读回确认旧值计数为 0 后，再用 `+field-update` 移除旧选项或调整最终顺序。

不要直接用只包含新选项的 `+field-update` 替换已有 select 定义；如果旧值还在记录中，这会让存量数据丢失或变空。
