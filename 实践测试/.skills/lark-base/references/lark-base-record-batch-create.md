...
- 用户明确要求演示/mock/测试记录数量时，`create_records` 数量必须严格等于用户指定数量。没有明确数量但属于搭建型任务时，默认写入 5~10 条有业务意义的示例数据。
- 写入后需要确认数量或用于交付验收时，用 `+record-list` 按目标表/视图读回，不要只凭创建命令成功就声称数据量符合要求。
