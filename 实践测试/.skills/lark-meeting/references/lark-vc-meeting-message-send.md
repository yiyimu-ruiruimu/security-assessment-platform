...
```bash
lark-cli vc +meeting-message-send --meeting-id <meeting_id> --text "稍等，我在看文档"
```
...
```bash
lark-cli vc +meeting-message-send --meeting-id <meeting_id> --msg-type reaction --emoji-type LOVE
lark-cli vc +meeting-message-send --meeting-id <meeting_id> --msg-type reaction --emoji-type VC_NoSound
```
...
3. 匹配到唯一会议后取长数字 `meeting_id`，再执行 `+meeting-message-send`。
...
- 需要 `vc:meeting.message:write` 权限；权限不足时，直接根据错误响应中的提示（如 `missing_scopes`、`console_url`）引导用户在应用后台补开权限后重试。
