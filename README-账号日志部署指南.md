...
| 文件 | 说明 |
|---|---|
| `index.html` | 登录拦截 + 行为记账调用点；**测评整改**：单元测评按单元顺序串行解锁、每单元 10 题（知识3·实验4·素养3，满分100通过）、账号学习位置记忆（登录自动续学）、实践闯关统一入口横幅（时间线正下方） |
...
2. **每单元 10 题**：知识 3 题 + 实验 4 题 + 素养 3 题，每题 10 分，满分 100；旧版 6 题存档自动兼容（首次登录迁移，不丢历史）。
...
## 七、在线互动答题（在线选择题 + 教师端统计）· 本次新增

### 功能说明
- **教师端（admin.html）**：新增「🎯 互动答题 · 在线选择题」卡片，两个标签页：
  - **📝 出题与管理**：在线创建题组（题干 + 2~6 个选项 + 正确答案，最多 30 题），一键「保存并开放作答 / 关闭作答」，可复制学生答题链接（`quiz-live.html?quiz=题组ID`）。
  - **📊 答题统计**：选择题组查看实时统计——每道题每个选项的**作答人数与百分比条形图**（正确答案选项标绿）、参与人数、学生作答明细表（每题对错、正确题数、提交时间），支持**自动刷新（10 秒）**和**导出明细 CSV**，适合课堂投屏。
- **学生端（quiz-live.html，新页面）**：登录账号后看到「进行中的答题」，点进入作答、提交；重复提交以最后一次为准；支持老师分享的直达链接。
- 权限：学生看不到正确答案；统计数据仅 ADMIN_EMAILS 中的教师账号可见。

### 部署（在原有部署基础上，仅需 1 步）
1. **建表**：登录 Supabase 控制台 → SQL Editor → 粘贴执行 `supabase-setup.sql` 末尾的「【三、互动答题】」建表语句（quiz_questions / quiz_answers 两张表，已含索引与 RLS）。
2. 推送代码即可（新增 `netlify/functions/quiz.js`、`quiz-live.html`，更新 `admin.html`、`index.html`、`_shared/backend.js`），无需新增环境变量。

### 文件清单
| 文件 | 变更 |
| --- | --- |
| `supabase-setup.sql` | 追加 quiz_questions / quiz_answers 建表语句 |
| `netlify/functions/_shared/backend.js` | sbSelect 支持 quiz_id/status 过滤；新增 sbDelete |
| `netlify/functions/quiz.js` | 新增云函数：出题/提交/统计 |
| `quiz-live.html` | 新增：学生在线作答页 |
| `admin.html` | 新增：互动答题卡片（出题 + 统计） |
| `index.html` | 头部新增「✋ 在线互动答题」「🧑‍🏫 教学管理后台」入口 |
